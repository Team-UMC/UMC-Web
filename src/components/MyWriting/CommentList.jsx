...
const MyList = () => {
  const dataList = [
    ...
    { id: 7, title: "일곱번째 글", comments: "ㅇㅇ", commentDate: "2024-01-21" },
    { id: 8, title: "여덟번째 글", comments: "우왕", commentDate: "2024-01-21" },
    { id: 9, title: "아홉번째 글", comments: "우왕", commentDate: "2024-01-21" },
    { id: 10, title: "열번째 글", comments: "우왕", commentDate: "2024-01-21" },
    { id: 11, title: "열한번째 글", comments: "우왕", commentDate: "2024-01-21" },
    { id: 12, title: "열두번째 글", comments: "우왕", commentDate: "2024-01-21" },
    { id: 13, title: "열세번째 글", comments: "우왕", commentDate: "2024-01-21" },
    { id: 14, title: "열네번째 글", comments: "우왕", commentDate: "2024-01-21" },
    { id: 15, title: "열다섯번째 글", comments: "우왕", commentDate: "2024-01-21" },

  ];

  const pageSize = 10; /*한 페이지안에 나타나는 글의 수 */
  const [currentPage, setCurrentPage] = useState(1); /*현재 표시되고 있는 페이지를 추적, 초기값은 1로 */
  
  const [searchTerm, setSearchTerm] = useState(""); /*사용자가 입력한 검색어를 저장하는데 사용 */
  const [searchedData, setSearchedData] = useState(null); /*검색 결과를 저장하기 위해 사용 */

  const lastIndex = currentPage * pageSize; /*현재 페이지에서 표시되는 마지막 항목의 인덱스 */
  const firstIndex = lastIndex - pageSize; /*현재 페이지에서 표시되는 첫번째 항목의 인덱스 */

  const currentList = searchedData
    ? searchedData.slice(firstIndex, lastIndex)
    : dataList.slice(firstIndex, lastIndex); /*현재 페이지에 표시될 항목의 목록을 검색 */
    ...
    <ListContainer>
      ...
      {currentList.map((item) => (
        <ListItem key={item.id}>
          ...
          {item.commentDate && (
            <Center>{item.commentDate}</Center>
          )}
        </ListItem>
      ))}
