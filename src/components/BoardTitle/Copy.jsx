...
const Copy = () => {
  ...
  useEffect(() => {
    ...
    const hostItem = urlParts[4];
    const boardItem = urlParts[5];
  
    const findHost = HOST_LISTS.find((host) => host.path === hostItem);
    const findBoard = BOARD_LISTS.find((board) => board.path === boardItem);
  
    setHostType(findHost);
    setBoardType(findBoard);
  }, []);

  // 카테고리와 게시판 이름이 없으면 null 반환
  if (!hostType || !boardType) {
    return null;
  }
  ...
  return (
    <BoardTitleContainer>
      <div className="board-title-icon">
        <img src={boardType.image} alt={`${boardType.path}-icon`} />
      </div>
      <BoardTitleWrapper>
        <BoardTitleMain>{boardType.title}</BoardTitleMain>
        <BoardTitleSub>{boardType.subtitle}</BoardTitleSub>
      </BoardTitleWrapper>
    </BoardTitleContainer>
  );
};
